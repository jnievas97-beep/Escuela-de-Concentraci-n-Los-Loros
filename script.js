...
function avisoEstaVigente(aviso, ahora = new Date()) {
    ...
    /* 1) Preferimos las fechas explícitas que genera GitHub Actions. */
    const desdeTexto =
        aviso && typeof aviso === "object"
            ? String(aviso.desde || "").trim()
            : "";

    const hastaTexto =
        aviso && typeof aviso === "object"
            ? String(aviso.hasta || "").trim()
            : "";

    let desde = null;
    let hasta = null;

    if (/^\d{4}-\d{2}-\d{2}$/.test(desdeTexto)) {
        desde = new Date(desdeTexto + "T00:00:00");
    }

    if (/^\d{4}-\d{2}-\d{2}$/.test(hastaTexto)) {
        hasta = new Date(hastaTexto + "T23:59:59.999");
    }

    /* 2) Si el JSON no trae fechas, las intentamos leer del nombre. */
    if (!desde && !hasta) {
        const vigenciaNombre = obtenerVigenciaDesdeNombreAviso(
            obtenerTextoIdentificadorAviso(aviso)
        );

        if (vigenciaNombre) {
            desde = vigenciaNombre.desde;
            hasta = vigenciaNombre.hasta;
        }
    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    /*
     * Mantener fija la posición al cargar o navegar entre secciones.
     * Evita que el navegador restaure una posición anterior.
     */
...
/* =====================================================
    ...
    /*
     * Evita el pequeño desplazamiento visual al cambiar entre
     * páginas/secciones. Funciona igual en PC y móvil.
     */
    function fijarPaginaArribaSinAnimacion() {
...
/* =====================================================
    ...
    async function cargarAvisoImportante() {
        ...
        if (avisoVigente) {
            ...
            if (botonAvisoImportante) {
                botonAvisoImportante.hidden = true;
                botonAvisoImportante.removeAttribute("href");
                delete botonAvisoImportante.dataset.rutaAviso;
                delete botonAvisoImportante.dataset.tipoAviso;
            }
        }
...
(function instalarEstiloAmpliarNoticias(){
    ...
    /* CONVIVENCIA Y BIENESTAR ESCOLAR - ACCESOS DE ORIENTACIÓN */
    document.querySelectorAll("[data-bienestar-pagina]").forEach(function (boton) {

        boton.addEventListener("click", function () {

            const pagina =
                boton.getAttribute("data-bienestar-pagina");

            const destino =
                boton.getAttribute("data-bienestar-destino");
    ...
    */
    ...
    function navegarSeguro(pagina,destino){
        ...
        try{
            ...
            window.setTimeout(function(){
                /*
                 * Solo desplazarse cuando existe un destino interno explícito.
                 * Si se cambia únicamente de pestaña, mantener la página arriba.
                 */
                if(destino){
                ...
                }else if(typeof fijarPaginaArribaSinAnimacion==="function"){
                    fijarPaginaArribaSinAnimacion();
                }else{
                    window.scrollTo(0,0);
    ...
    document.addEventListener("click",function(e){
        ...
        /*
         * El menú principal ya posee su propio evento de navegación.
         * Evitar una segunda ejecución que cause desplazamiento visual.
         */
        if(b.classList.contains("boton-menu")){
            return;
        }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const motivoJustificacion =
        document.getElementById("justificacionMotivo");

    const respaldoLabel =
        document.getElementById("justificacionRespaldoLabel");

    const respaldoAyuda =
        document.getElementById("justificacionRespaldoAyuda");

    function requiereRespaldoMedico() {
        const motivo =
            motivoJustificacion
                ? motivoJustificacion.value.trim()
                : "";

        return (
            motivo === "Motivo de salud" ||
            motivo === "Atención médica"
        );
    }

    function actualizarObligatoriedadRespaldo() {
        const obligatorio = requiereRespaldoMedico();

        if (archivoJustificacion) {
            archivoJustificacion.required = obligatorio;
            archivoJustificacion.setAttribute(
                "aria-required",
        ...
        if (respaldoLabel) {
            respaldoLabel.textContent = obligatorio
                ? "Documento de respaldo *"
                : "Documento de respaldo (opcional)";
        }

        if (respaldoAyuda) {
            respaldoAyuda.textContent = obligatorio
                ? "Obligatorio para Motivo de salud o Atención médica. Adjunte PDF, JPG, PNG o WEBP, con un máximo de 5 MB."
                : "Puede adjuntar PDF, JPG, PNG o WEBP, con un máximo de 5 MB.";
        }
    }

    if (motivoJustificacion) {
        motivoJustificacion.addEventListener(
            "change",
            actualizarObligatoriedadRespaldo
        );
    }
    ...
    formularioJustificacion.addEventListener("submit", async function (evento) {
        ...
        if (requiereRespaldoMedico() && !archivo) {
            mostrarResultadoJustificacion(
                "error",
                "Para Motivo de salud o Atención médica debe adjuntar un documento de respaldo."
            );

            if (archivoJustificacion) {
                archivoJustificacion.focus();
            }
    ...
    function descargarComprobanteJustificacion(datos) {
        ...
        const LOGO_COMPROBANTE = "./imagenes/logo.png";
        ...
        logo.onload = function () {
            ...
            ctx.fillText(
                "Este comprobante acredita que la justificación fue enviada correctamente.",
                125,
                920
            );
            ...
            ctx.fillText(
                "Conserve esta imagen como respaldo del envío.",
                125,
                950
            );
...
/* =========================================================
   POSICIÓN FIJA DE NAVEGACIÓN · 20260902-9
   Evita pequeños saltos al terminar de cargar la página.
========================================================= */
window.addEventListener("load", function () {
    const html = document.documentElement;
    const body = document.body;
...
/* =========================================================
   NOTIFICACIONES PÚBLICAS FCM · FASE 1 COMPLETA · 20260907-4
   Noticias · Comunicados · Avisos importantes
   Resincronización automática del token al abrir/volver a la PWA.
========================================================= */
(function () {
    "use strict";
    ...
    const VAPID_PUBLICA = "BGdd3M34jY-u5u9zf3Nq19o8NCCldRCEoMi7gitbea-_aUTel-nslISWJ4hqcIRzAaQDbYlEnfefaQSyxVdDsC8";
    const CLAVE_TOKEN_LOCAL = "escuelaLosLorosFcmToken";
    const CLAVE_ULTIMO_REGISTRO = "escuelaLosLorosFcmUltimoRegistro";
    const ENDPOINT_REGISTRO = (window.ESCUELA_NOTIFICACIONES && window.ESCUELA_NOTIFICACIONES.endpointRegistro) || "";

    // Evita llamadas repetidas por focus/pageshow/visibilitychange casi simultáneos.
    // No reemplaza el registro en Apps Script: cada nueva apertura real vuelve a sincronizar.
    const INTERVALO_REINTENTO_SESION = 60 * 1000;
    let ultimaSincronizacionSesion = 0;
    let sincronizacionEnCurso = null;
    let onMessageInstalado = false;

    function mostrarToastNotificaciones(texto) {
        const toast = document.getElementById("toastSitio");
        if (!toast) { alert(texto); return; }
        toast.textContent = texto;
        toast.hidden = false;
        window.clearTimeout(mostrarToastNotificaciones.temporizador);
        mostrarToastNotificaciones.temporizador = window.setTimeout(function () { toast.hidden = true; }, 5200);
    }
    ...
    async function registrarTokenPublico(token) {
        if (!ENDPOINT_REGISTRO || ENDPOINT_REGISTRO.indexOf("script.google.com/macros/s/") === -1) {
            console.warn("Falta configurar el endpoint público de notificaciones.");
            return false;
        }

        const ahora = Date.now();
        const cuerpo = new URLSearchParams();
        cuerpo.set("accion", "suscribir");
        cuerpo.set("token", token);
        cuerpo.set("origen", location.origin + location.pathname);
        cuerpo.set("plataforma", esIOS() ? "iOS/iPadOS" : (navigator.userAgent || "").slice(0, 180));

        await fetch(ENDPOINT_REGISTRO, {
            method: "POST",
            mode: "no-cors",
        ...
        localStorage.setItem(CLAVE_ULTIMO_REGISTRO, String(ahora));
        return true;
    }

    function instalarRecepcionEnPrimerPlano(messaging) {
        if (onMessageInstalado || !messaging) return;
        onMessageInstalado = true;
        messaging.onMessage(function (payload) {
            const datos = payload && payload.data ? payload.data : {};
            const notif = payload && payload.notification ? payload.notification : {};
            const titulo = datos.titulo || notif.title || "Escuela de Concentración Los Loros";
            const cuerpo = datos.cuerpo || notif.body || "Nuevo aviso disponible.";
            mostrarToastNotificaciones("🔔 " + titulo + ": " + cuerpo);
        });
    }

    async function obtenerTokenYRegistrar() {
        if (!("serviceWorker" in navigator) || !window.firebase) return "";
        if (!firebase.apps.length) firebase.initializeApp(FIREBASE_CONFIG);

        const messaging = firebase.messaging();
        instalarRecepcionEnPrimerPlano(messaging);

        const registro = await navigator.serviceWorker.ready;
        const token = await messaging.getToken({
            vapidKey: VAPID_PUBLICA,
            serviceWorkerRegistration: registro
        });

        if (!token) throw new Error("Firebase no entregó un token de registro.");

        const anterior = localStorage.getItem(CLAVE_TOKEN_LOCAL) || "";
        localStorage.setItem(CLAVE_TOKEN_LOCAL, token);

        // Siempre vuelve a informar el token vigente a Apps Script.
        // Si Firebase lo cambió, se registra el nuevo; si es el mismo, se actualiza su fecha y queda Activo.
        await registrarTokenPublico(token);

        if (anterior && anterior !== token) {
            console.info("FCM renovó el token de este dispositivo y fue registrado nuevamente.");
        }

        return token;
    }

    async function sincronizarTokenAutomaticamente(forzar) {
        if (!("Notification" in window) || Notification.permission !== "granted") return false;
        if (!navigator.onLine) return false;

        const ahora = Date.now();
        if (!forzar && ahora - ultimaSincronizacionSesion < INTERVALO_REINTENTO_SESION) return true;
        if (sincronizacionEnCurso) return sincronizacionEnCurso;

        ultimaSincronizacionSesion = ahora;
        sincronizacionEnCurso = (async function () {
            try {
                await obtenerTokenYRegistrar();
                actualizarEstadoBotones();
                return true;
            } catch (e) {
                console.warn("No fue posible resincronizar FCM:", e);
                return false;
            } finally {
    ...
    async function activarNotificaciones() {
        const boton = document.getElementById("botonNotificacionesPublicas");
        if (!("Notification" in window) || !("serviceWorker" in navigator) || !window.firebase) {
            mostrarToastNotificaciones("Este navegador no permite activar las notificaciones de la aplicación.");
            return;
        }
        if (esIOS() && !estaEnModoApp()) {
            mostrarToastNotificaciones("En iPhone/iPad, primero agrega la página a la pantalla de inicio y ábrela desde su icono.");
            return;
        }
        ...
        try {
            if (boton) {
                boton.disabled = true;
                const t = boton.querySelector("span:last-child");
                if (t) t.textContent = "Activando…";
            }

            const permiso = await Notification.requestPermission();
            if (permiso !== "granted") {
                mostrarToastNotificaciones("No se activaron las notificaciones. Puedes autorizarlas desde la configuración del navegador.");
                return;
            }

            await sincronizarTokenAutomaticamente(true);
            actualizarEstadoBotones();
            mostrarToastNotificaciones("Notificaciones activadas. Desde ahora recibirás Noticias, Comunicados y Avisos Importantes de la Escuela.");
        } catch (error) {
            console.error("Error al activar notificaciones:", error);
            mostrarToastNotificaciones("No fue posible activar las notificaciones. Revisa la conexión e inténtalo nuevamente.");
        } finally {
    ...
    function actualizarEstadoBotones() {
        ...
        const token = localStorage.getItem(CLAVE_TOKEN_LOCAL);

        if (Notification.permission === "granted" && token) {
            boton.classList.add("notificaciones-activas");
            boton.hidden = true;
            if (texto) texto.textContent = "Notificaciones activadas";
        } else {
            boton.classList.remove("notificaciones-activas");
            boton.hidden = false;
            if (texto) texto.textContent = "Activar notificaciones";
        }
    }

    function conectarResincronizacionAutomatica() {
        // Volver a la PWA desde segundo plano.
        window.addEventListener("pageshow", function () {
            sincronizarTokenAutomaticamente(false);
        });
        ...
        // Si el teléfono recupera Internet, vuelve a registrar el token.
    ...
    document.addEventListener("DOMContentLoaded", function () {
        const boton = document.getElementById("botonNotificacionesPublicas");
        const esTelefono = window.matchMedia("(max-width: 768px)").matches;
        ...
        if (boton) boton.addEventListener("click", activarNotificaciones);
        actualizarEstadoBotones();
        conectarResincronizacionAutomatica();

        // IMPORTANTE: si el permiso ya existe, cada nueva apertura de la PWA
